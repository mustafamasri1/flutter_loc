...
Future<void> generateVisitLog(String sourcingDirectory,
    Map<String, List<LocMatch>> findResults, String generationDirectory,
    {List<RegExp>? filesToExclude, List<String>? extensionsToInclude}) async {
  ...
  List<File> filesVisited = await IOHelper.file.listAll(
    ...
    allowed: [
      RegExp(r'.*\.dart$'),
      ...?extensionsToInclude
          ?.map((p) => RegExp(r'.*' '${RegExp.escape(p)}' r'$')),
    ],
  );
  ...
  for (var file in filesVisited) {
    if (findResults.keys
        .map((i) => i.clearPathTrails)
        .toList()
        .contains(Uri.file(file.absolute.path).path)) {
      visitMap.addEntries([
        MapEntry(
            file.uri.pathSegments.last,
            findResults
                    .map((i, v) => MapEntry(i.clearPathTrails, v))[
                        Uri.file(file.absolute.path).path]!
                    .map((a) => a.matchesInLine.length)
                    .toList()
                    .reduceIfNotEmpty((aa, bb) => aa + bb) ??
                0)
      ]);
    }
  }
  ...
  await IOHelper.file.writeString(
      "$generationDirectory/visit_log.txt",
      visitMap.entries
              .map((entry) => "${entry.key}  ===>  (${entry.value} matches)")
              .toList()
              .reduceIfNotEmpty((a, b) => "$a\n$b") ??
          'No Matches!');
}

...
Future<Map<String, List<LocMatch>>> findHardcodedStrings(
  ...
  final String tripleDoubleQuoted =
      r'"""(?:(?!""")[\s\S])*?"""'; // Non-greedy triple double-quoted strings
  final String tripleSingleQuoted =
      r"'''(?:(?!''')[\s\S])*?'''"; // Non-greedy triple single-quoted strings
...
// Combine patterns with alternation
  ...
  ret = Map.fromEntries(searchResult.entries.map((entry) => MapEntry(
      Uri.directory(Directory(entry.key).absolute.path).path,
      entry.value
          .map((v) => LocMatch(

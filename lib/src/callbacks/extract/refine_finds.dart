...
/// Refine the finds.
Future<Map<String, List<LocMatch>>> refineFinds(Map<String, List<LocMatch>> inputFinds, {String? customRefinementLogicPath}) async {
  ...
  if (customRefinementLogicPath != null) {
    ...
    refinementIsolate = RefinementIsolate(IOHelper.directory.getCurrent() + Platform.pathSeparator + customRefinementLogicPath.replaceSeparator());
  }
  //----
  await Future.forEach(inputFinds.entries, (inputEntry) async {
    ...
    await Future.forEach(outputFinds.entries, (findEntry) async {
      ...
      await Future.forEach(matches, (matchValue) async {
        Map<int, String> newM = {};
        await Future.forEach((matchValue.matchesInLine.entries), (mapEntry) async {
          //1.Remove strings followed by tr()
          bool isFollowedByTr = matchValue.lineContent.length > ((mapEntry.key) + mapEntry.value.length) && matchValue.lineContent.substring(mapEntry.key + mapEntry.value.length).startsWith(r'.tr()');

          //2. Apply custom refinement logic.
          bool failsCustomRefinement = true;
          if (refinementIsolate != null) {
            failsCustomRefinement = !await refinementIsolate.applyCustomRefinement(
              inputEntry.key,
        ...
        matches[matches.indexOf(matchValue)] = matchValue.copyWith(matchesInLine: newM);
      });

...
Future<Map<String, List<LocReplacement>>> parseFileContent(
  ...
  List<String> matchedSearchFiles = RegExp(
          '${fileStartDelimeter('(.*?)')}(.*?)$fileEndDelimeter',
          dotAll: true)
      .allMatches(fileData)
  ...
  await Future.forEach(matchedSearchFiles, (searchFile) async {
    // 1. Got the file path
    String? filePath = RegExp(fileStartDelimeter('(.*?)'))
        .allMatches(searchFile)
        .first
        .group(1);
    if (filePath != null) {
      // Get the matching content block
      String? matchContentBlock = RegExp(
      ...
      if (matchContentBlock != null) {
        ...
        List<String> matchedLineNumbers =
            RegExp(matchesToMatchesDelimeter(r'(\d+?)'))
                .allMatches(matchContentBlock)
                .map((a) => a.group(1))
                .where((a) => a != null)
                .map((aa) => aa!)
                .toList();
        List<String> matchedLineBlocks = matchContentBlock
            .split(RegExp(matchesToMatchesDelimeter(r'(\d+?)')))
        ...
        if (matchedLineNumbers.length == matchedLineBlocks.length) {
          ...
          await Future.forEach(matchedLineBlocks, (lineBlock) async {
            ...
            int lineNumber = int.tryParse(
                    matchedLineNumbers[matchedLineBlocks.indexOf(lineBlock)]) ??
                -1;

            List<String> positionMatcheBlocks =
                RegExp(r'(\(\d+?\))(.*?)' '$lineEndDelimeter', dotAll: true)
                    .allMatches(lineBlock)
                    .map((m) => m.group(0))
                    .where((a) => a != null)
                    .map((aa) => aa!)
                    .toList();
            ...
            await Future.forEach(positionMatcheBlocks, (positionMatch) async {
              ...
              int positionNumber = int.tryParse(RegExp(r'(\(\d+?\))')
                          .firstMatch(positionMatch)
                          ?.group(1)
                          .toString()
                          .replaceFirst('(', '')
                          .replaceFirst(')', '') ??
              ...
              String? s = RegExp(r'(\(\d+?\)) (.*?)' '$lineEndDelimeter',
              ...
              if (positionNumber != -1 &&
                  whatToReplace.isNotEmpty &&
                  whatToReplaceItWith.isNotEmpty) {
                positionsReplacementsMap.addEntries([
                  MapEntry(positionNumber, (whatToReplace, whatToReplaceItWith))
                ]);
              }
            });

            if (positionsReplacementsMap.isNotEmpty && lineNumber != -1) {
              replacementsList.add(LocReplacement(
                  linePosition: lineNumber,
                  matchesInLine: positionsReplacementsMap));
            }
          });
